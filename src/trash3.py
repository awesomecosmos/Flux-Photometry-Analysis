# -*- coding: utf-8 -*-
"""
Created on Thu Oct 21 08:51:20 2021
...
# i = 0
    ...
    # for i in range(len(moa_sources)):
    #     error = np.mean(calc_total_error(moa_sources['flux'][i], bkg.background_rms, effective_gain))
 ...
 #%%    
    ...
    def sm_to_moa_transform(sm_sources,moa_sources):
        ...
        for i in range(len(t1)):
            ...
            if np.abs(t1['xcenter'].value[i] - s2['ra'].values[i]) >= 0.9*min_value[i]:
...
comet_pixel_location_x = 1151
comet_pixel_location_y = 2344
